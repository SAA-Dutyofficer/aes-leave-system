  ...
  let html = `<div class="roster-table-wrap"><table class="roster-table">
    <thead>
      <tr>
        ...
        ${dateHeaders.map(h=>`<th class="roster-day-hdr${h.isWeekend?" wk-col":""}" ${h.isWeekend?'style="background:#4a6fa5;color:#ffffff;"':''}>${h.d}<br/><span class="roster-wd">${h.wdLabel}</span></th>`).join("")}
      </tr>
    </thead>
    <tbody>`;
...
function getCellStyle(type, isWeekend) {
  const S = {
    D:    "background:#dbeafe;color:#1e40af;font-weight:700;",
    N:    "background:#ede9fe;color:#5b21b6;font-weight:700;",
    "48": "background:#c7d7fe;color:#1e3a8a;font-weight:700;",
